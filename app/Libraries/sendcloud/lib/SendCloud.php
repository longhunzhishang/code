...
class SendCloud {
	...
	protected function wrapParam_v2(Maill $mail) {
		$param = array ();
		if ($this->api_user) {
	...
	protected function wrapParam(Maill $mail) {
		if ($this->version == 'v1') {
	...
	protected function wrapParam_v1(Maill $mail) {
		$param = array ();
		if ($this->api_user) {
	...
	protected function wrapBody(Maill $mail) {
		if ($this->version == 'v1') {
	...
	protected function wrapBody_v1(Maill $mail) {
		$header = '';
	...
	protected function wrapBody_v2(Maill $mail) {
		$header = '';
		$paramArray = $mail->jsonSerialize ();
		$eol = "\r\n";
		$data = '';
	...
	public function sendCommon(Maill $mail) {
		global $config;
	...
	public function sendTemplate(Maill $mail) {
		global $config;
		$method = "POST";
		if ($mail->hasAttachment ()) {
			$bodyData = $this->wrapBody ( $mail );
			...
			$param = $this->wrapParam ( $mail );
			$config [$this->version] ['sendTemplate'] = "/webapi/mail.send_template.json";
