...
class SubjectsController extends Controller
{
    ...
    public function getInfoInIds(Request $request)
    {
        ...
        $sub_rs = HttpUtil::parse_curl_search(config('param.SUBJECT_SEARCH'),$curl_param);
        ...
        $url2 = config('param.ES_URL').config('param.ADDATA_SEARCH').'/'.config('param.ES_SEARCH').'?_source=id,type,attribute08';
        ...
        foreach ($data_sub['data'] as $key => &$value) {
            ...
            foreach ($data['data'] as $k => $v) {
                if($v && $v->type=='html5' && $value['ad_id']==$v->id)
                {
                    $value['thumbnail'] = $v->attribute08;
                    $value['type'] = 'html5';
                }
            }
        }
    ...
    public function getAll(Request $request)
    {
        ...
        $rs = HttpUtil::parse_curl_search(config('param.SUBJECT_SEARCH'),DSLUtil::baseDSLQuery(0,$size));
    ...
    public function getAll_3(Request $request)
    {
        ...
        $rs = HttpUtil::parse_curl_search(config('param.SUBJECT_SEARCH'),DSLUtil::baseDSLQuery($page * $size,$size));
        $data = DBUtil::get_parse_es_data($rs);
        // 暂无数据 2002
        if(empty($data['data']))return HttpUtil::retrun_json(['data'=>[],'status'=>203],200);
        $data['status'] = empty($data['data'])?203:200;
