...
class MainIndexController extends Controller
{
    ...
    public function getInfoId(Request $request)
    {
        ...
        $es = ESUtill4::getInfoById(config('param.MIANS_SEARCH6'),$id);
    ...
    public function getInfoInIds(Request $request)
    {
        ...
        foreach ($ads_id as $key => $value) {
            if($value)
            {
                $es = ESUtill4::getInfoById(config('param.MIANS_SEARCH6'),$value);
    ...
    public function getAllField(Request $request)
    {
        ...
        $es = ESUtill4::getFieldInfo(config('param.MIANS_SEARCH6'),$field,$param,$size);
    ...
    public function getCountByFiled(Request $request)
    {
        ...
        $url = config('param.ES_URL').config('param.MIANS_SEARCH6').'/'.config('param.ES_SEARCH');
    ...
    public function getInfoGroupByFiled(Request $request)
    {
        ...
        if($field=='tracker')
        {
            ...
            if($is_flag_domain[0]->flag==1 || $is_tag==1)
            {
             ...
             unset($url_query[$field]);
                ...
                $rs = HttpUtil::parse_curl_search(config('param.ADDATA_SEARCH6'),$curl_param_tr2);
        ...
        foreach ($data_new['data'] as $k => &$v) {
            ...
            // $ad_url['_source'] = config('param.ADDATA_FILED_SOURCE');
            $url = config('param.ES_URL').config('param.ADDATA_SEARCH6').'/'.config('param.ES_SEARCH').'?_source=id,type,attribute08,thumbnail';
