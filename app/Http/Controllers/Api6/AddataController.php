...
class AddataController extends Controller
{
    ...
    public function getAllNew(Request $request)
    {
        ...
        $url = config('param.ES_URL').config('param.ADDATA_SEARCH6').'/'.config('param.ES_SEARCH').'?_source=id,domain,title,advertiser,publisher,trackers,thumb_url';
        // if($type)$url = $url.'?_source='.config('param.ADDATA_FILED');
    ...
    public function getAllSubject(Request $request)
    {
        ...
        // }
         ...
         foreach ($data['subject'] as $key => &$value) {
            ...
            // dd(json_encode($ad_info_config));
            $url = config('param.ES_URL').config('param.ADDATA_SEARCH6').'/'.config('param.ES_SEARCH').'?_source=id,advertiser,type,attribute08,am_url,am_source_url,fingerprint';
            $rs_ad_domain = HttpUtil::send_curl_methods($url,$ad_info_config,"POST");
            ...
            foreach ($domain_advertiser['data'] as $k => $v) {
                ...
                $value['thumb_url'] = $v['am_url'];
                $value['am_url'] = $v['am_url'];
                $value['am_source_url'] = $v['am_source_url'];
                $value['type'] = $v['type'];
            }
    ...
    public function getCount(Request $request)
    {
        ...
        if(empty($param))unset($curl_param['query']);
        $url = config('param.ES_URL').config('param.ADDATA_SEARCH6').'/'.config('param.ES_SEARCH').'?search_type=count';
        $rs = HttpUtil::send_curl_methods($url,$curl_param,"POST");
    ...
    public function getInfoByMd5(Request $request)
    {
        ...
        $es = ESUtil::getInfoByMd5(config('param.ADDATA_SEARCH6'),$md5);
        $data['data'] = $es;
        $data['status'] = empty($es)?203:200;
        return HttpUtil::retrun_json($data,200);
    }
    ...
    public function getInfoInIds(Request $request)
    {
        ...
        //     if($value)
        //     {
        //         $es = ESUtil::getInfoById(config('param.ADDATA_SEARCH6'),$value);
    ...
    public function getWHADsTmp(Request $request)
    {
        ...
        // dd(json_encode($curl_param));
        $url = config('param.ES_URL').config('param.ADDATA_SEARCH6').'/'.config('param.ES_SEARCH').'?scroll=3m';
