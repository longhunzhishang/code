...
class ApiController extends Controller
{
    ...
    public function getOttListInfo(Request $request)
    {
        $ids = empty($request->ids)?[]:$request->ids;
        $ids = explode('_OID_', $ids);

        if(empty($ids))return HttpUtil::retrun_json(HttpUtil::returnStatus(206),200);
    ...
    public function bulkEsData(Request $request)
    {
        ...
        switch ($type) {
            case '1':
                ...
                break;
            case '2':
                $search = config('param.MIANS_SEARCH6');
                break;
            case '3':
                $search = config('param.SUBJECT_SEARCH6');
                break;
            case '4':
        ...
        foreach ($ad as $k => &$v) {
            ...
            $url = config('param.ES_URL').config('param.ADDATA_SEARCH6').'/'.$v['id'];
    ...
    public function addDomainInfo()
    {
      ...
      //  $end_time = strtotime($before_day_end_time).'000'; // 前一天 结束时间
        ...
        ->join('addata_2018 as addata','addata_rank_role.ad_id','=','addata.id')
    ...
    public function getDomainByAd(Request $request)
    {
        ...
        if($cul_role=='subject')
        {
            ...
            $url_sub = config('param.ES_URL').config('param.SUBJECT_SEARCH6').'/'.config('param.ES_SEARCH');
    ...
    public function getSubTitle($subjects_tmp)
    {
        ...
        $rs_re_ads = HttpUtil::parse_curl_search(config('param.SUBJECT_SEARCH6'),$curl_param);
    ...
    public function getSearchRs(Request $request)
    {
        ...
        foreach ($sub_data_tmp as $k => &$v) {
            ...
            $rs = HttpUtil::parse_curl_search(config('param.MIANS_SEARCH6'),$curl_param_sub);
            ...
            foreach ($mi_arr as $k_m => $v_m) {
                    ...
                    $rs = HttpUtil::parse_curl_search(config('param.ADDATA_SEARCH6'),$curl_param_m);
    ...
    public function getInfoByRole(Request $request)
    {
        ...
        $rs = HttpUtil::parse_curl_search(config('param.ADDATA_SEARCH6'),$curl_param_ad);
        ...
        $item['advertisers'] = empty($da['aggregations']['advertisers']['value'])?'0':$da['aggregations']['advertisers']['value'];
        $item['publishers'] = empty($da['aggregations']['publishers']['value'])?'0':$da['aggregations']['publishers']['value'];
        $item['trackers'] = empty($da['aggregations']['trackers']['value'])?'0':$da['aggregations']['trackers']['value'];
        $item['risk'] = empty($da['aggregations']['risk']['value'])?'0':$da['aggregations']['risk']['value'];
        ...
        $rs = HttpUtil::parse_curl_search(config('param.ADDATA_SEARCH6'),$curl_param_d);
        $rs_des = json_decode($rs,true);
    ...
    {
        $rs = ESUtil::getInfoById(config('param.ADDATA_SEARCH6'),$id);
...
//        return HttpUtil::retrun_json($data,200);
    ...
    protected function getRecentVideoAds(Request $request)
    {
        ...
        $url = config('param.ES_URL').config('param.ADDATA_SEARCH6').'/'.config('param.ES_SEARCH').'?_source='.config('param.ADDATA_FILED');
        $rs = HttpUtil::send_curl_methods($url,$curl_param,"POST");
        $data = DBUtil::get_parse_es_data($rs);
        if(empty($data['data'])) return HttpUtil::retrun_json(['data'=>[],'status'=>203],200);
    ...
    public function ppku(Request $request)
    {
        ...
        $rs_re = HttpUtil::parse_curl_search(config('param.ADDATA_SEARCH6'),$curl_param);
